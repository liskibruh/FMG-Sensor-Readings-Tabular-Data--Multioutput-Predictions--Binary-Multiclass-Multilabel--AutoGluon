...
train_data = TabularDataset('../input/fmg-sensor-data/training_dataset.csv')
subsample_size = 2000  # subsample subset of data for faster demo, try setting this to much larger values
train_data = train_data.sample(n=subsample_size, random_state=0)
train_data.head()

labels = ['Gesture','Position']  # which columns to predict based on the others
...
multi_predictor = MultilabelPredictor(labels=labels, path=save_path, eval_metrics=eval_metrics)
multi_predictor.fit(train_data, time_limit=time_limit)

test_data_pre=pd.read_csv('../input/fmg-sensor-data/FMG_SensorDataReadings-cleaned.csv')
test_data=pd.DataFrame(test_data_pre)
test_data['Position'] = 0
test_data[['Unnamed: 0', 'Gesture']] = 0
test_data = TabularDataset(test_data)
#test_data = TabularDataset('C:/Users/Owais Tahir/Documents/Spyder-Dir/FMG_SensorDataReadings-cleaned.csv')
...
test_data_nolab=test_data_nolab.dropna(how= "any")
...
predictions_df=pd.DataFrame(predictions) #converting predictions to dataframe so we can download it in csv format in the next line
